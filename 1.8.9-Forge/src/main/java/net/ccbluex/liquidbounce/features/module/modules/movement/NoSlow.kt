...
class NoSlow : Module() {
    ...
    private val packetModeValue = ListValue("PacketMode", arrayOf("Packet","AntiCheat","Custom","NCP","Vanilla","AAC","Ghost"), "NCP")
    // Soulsand
    ...
    private fun sendPacket(Event : MotionEvent,SendC07 : Boolean, SendC08 : Boolean,Delay : Boolean,DelayValue : Long,onGround : Boolean,Hypixel : Boolean = false) {
        ...
        if(SendC08 && Event.eventState == EventState.PRE) {
        ...
        when(packetModeValue.get().toLowerCase()) {
            ...
            "packet" -> {
                ...
                if(mc.thePlayer.ticksExisted % 2 == 0) {
                    this.sendPacket(event, SendC07 = false, SendC08 = true, Delay = true, DelayValue = 0, onGround = true, Hypixel =false)
                }
            }
            "Ghost" -> {
                if (event.eventState == EventState.PRE) {
