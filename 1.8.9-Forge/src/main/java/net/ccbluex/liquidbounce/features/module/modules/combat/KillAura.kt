...
import Core.C08Packet.utils.render.EaseUtils
...
import net.ccbluex.liquidbounce.utils.RotationUtil
...
class KillAura : Module() {
    ...
    // AutoBlock
     ...
     private val autoBlockModeValue = ListValue("AutoBlockMode", arrayOf("AfterTick", "AfterAttack", "Vanilla"),"AfterTick")
    private val autoBlockPacketValue = ListValue("AutoBlockPacketMode", arrayOf("Packet", "AAC", "Vanilla"),"Packet")
    private val rotationModeValue = ListValue("RotationMode", arrayOf("BackTrack", "Vanilla","Basic", "Simple"), "Simple")
    ...
    fun onMotion(event: MotionEvent) {
        ...
        if (event.eventState == EventState.PRE) {
            // AutoBlock
            if (!autoBlockValue.get().equals("off",true) && discoveredTargets.isNotEmpty() && (!autoBlockModeValue.get().equals("AfterAttack",true)||discoveredTargets.filter { mc.thePlayer.getDistanceToEntityBox(it)>maxRange }.isNotEmpty()) && canBlock()) {
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        // Stop blocking
        if (!autoBlockModeValue.get().equals("Vanilla",true)&&(mc.thePlayer.isBlocking || blockingStatus)) {
        ...
        if (mc.thePlayer.isBlocking || (!autoBlockValue.get().equals("off",true) && canBlock())) {
            if(autoBlockModeValue.get().equals("AfterTick",true))
                return
    ...
    private fun updateRotations(entity: Entity): Boolean {
        ...
        val limitedRotation = RotationUtils.limitAngleChange(RotationUtils.serverRotation, when (rotationModeValue.get()) {
            "BackTrack" -> RotationUtils.toRotation(RotationUtils.getCenter(boundingBox), predictValue.get())
            "Basic" -> RotationUtil.getBasicRotations(entity)
            "Simple" -> RotationUtil.getRotationsForAura(entity, maxRange + 0.5f)
            "Normal" -> rotation
            else -> RotationUtil.getRotationsForAura(entity, maxRange + 0.5f)
        },
        ...
        when(autoBlockPacketValue.get().toLowerCase()) {
            ...
            "vanilla" -> {
                if (mc.thePlayer.heldItem.item is ItemSword) {
                    if (interact) {
                        mc.thePlayer.sendQueue.addToSendQueue(C02PacketUseEntity(target, C02PacketUseEntity.Action.INTERACT))
