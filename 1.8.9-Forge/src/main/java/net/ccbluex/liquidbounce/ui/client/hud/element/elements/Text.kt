...
import Core.C08Packet.HWID.Login
import Core.C08Packet.Main.WbxMain
import Core.C08Packet.utils.render.ColorManager
import Core.C08Packet.utils.render.Palette
...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    private fun getReplacement(str: String): String? {
        ...
        return when (str) {
            ...
            "bps" -> MovementUtils.getBlockSpeed(mc.thePlayer).toString()
    ...
    override fun drawElement(): Border? {
        ...
        if (oneTapRect.get()) {
            ...
            RenderUtils.drawGradientSideways(-3.0, -7.0, fontRenderer.getStringWidth(displayText) + 2.0, -3.0, if (colorMode == "Rainbow") rainbow(400000000L).rgb + Color(0, 0, 0, 50).rgb else addColor, if (colorMode == "Rainbow") rainbow(400000000L).rgb else color)
        }
        if (skeetRect.get()) {
        ...
        RainbowFontShader.begin(colorMode == "Rainbow", if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
            fontRenderer.drawString(displayText, 0F,0F, when {
                ...
                colorMode.equals("flux", ignoreCase = true) -> ColorManager.fluxRainbow(-100, counter[0] * -50 * rainbowSpeed.get().toLong(), rainbowSaturation.get())
                colorMode.equals("astolfo", ignoreCase = true) -> ColorManager.astolfoRainbow(counter[0] * 100, astolfoRainbowOffset.get(), astolfoRainbowIndex.get())
        ...
        if (editMode && mc.currentScreen is GuiHudDesigner && editTicks <= 40)
            fontRenderer.drawString("_", fontRenderer.getStringWidth(displayText) + 2F,
                    0F, if (colorMode == "Rainbow") ColorUtils.rainbow(400000000L).rgb else color, shadow.get())
